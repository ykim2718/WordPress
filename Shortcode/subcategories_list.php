...
function ajax_filter_my_posts_handler() {
    ...
    // 기본 쿼리 조건
    $where_clause = "tt.term_id IN ($in_clause) AND tt.taxonomy = 'category' AND p.post_status = 'publish' AND p.post_type = 'post'";

    // Currently Selling이 체크된 경우에만 키워드 필터 추가
    if ($selling_only === 'true') {
        $keyword = $wpdb->esc_like('sdks.shopifycdn.com');
        $like_clause = "'%" . esc_sql($keyword) . "%'";
        $where_clause .= " AND p.post_content LIKE $like_clause";
    }
