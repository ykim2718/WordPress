...
플러그인. Topic 을 누르면 그 topic 으로 센 글 목록이 열린다.
...
| `includes/search.php` | 낱말을 눌렀을 때 열리는 목록을 센 글로 바꿈 |
...
74 개를 세는 데 미러에서 4 초, 실제 사이트에서 23 초가 걸렸다.

그래서 두 가지를 한다. 센 것은 따로 하루 담아 두어 색이나 글자 크기를 바꿔도 다시 세지
않는다. 그리고 세는 일을 **topic 이 들어오는 요청에서** 미리 해 둔다. 하루 한 번의
가져오기, REST 로 올리기, 새로고침 단추가 모두 그 길을 지나므로 방문자가 그 시간을
기다리지 않는다.

## 누르면 열리는 목록

topic 의 글 수는 이름만이 아니라 구절로도 센다. 그런데 누르면 열리는 것이 이름으로 하는
보통 검색이면 구절로 걸린 글이 목록에 없다. tooltip 은 12 개라 하고 목록은 3 개인 일이
그래서 생긴다.

그래서 목록을 센 것에서 만든다. 주소에 `kwc=<TOPIC>` 으로 topic 의 본디 이름을 실어
보내고, 검색 질의를 그 topic 이 걸린 글로 바꾼다. 이름은 번역될 수 있으므로 보이는 이름이
아니라 본디 이름을 싣는다. 화면의 제목은 검색어 그대로 남는다.

`kwc` 가 없는 주소는 손대지 않는다. 사람이 직접 친 검색은 늘 하던 대로 돈다. 모르는 이름이
실려 오면 보통 검색으로 둔다.

## 감춘 분류

**RESTRICTED** 분류에 든 글은 구름에서 빠진다. 세는 자리에서 빠지고, 목록은 센 글로
만들므로 목록에서도 빠진다. 주소에는 `cat=-<TERM_ID>` 도 함께 실어, 보통 검색으로 돌아가는
경우에도 그 글이 나오지 않게 한다.

분류는 이름으로 찾는다. 슬러그는 사이트마다 다르게 붙지만 이름은 화면에 보이는 그대로다.
대소문자는 가리지 않고, 그 아래 분류에 든 글도 함께 빠진다.

파이프라인도 그 글을 읽지 않는다. `tools/llm_keywords.py` 가 그 분류의 글을 빼고 읽으므로
감춘 글의 말이 topic 이 되는 일이 없다.
...
마우스를 올리면 배경이 옅은 회색이 되고 글씨가 굵어진다. 배경까지 여기서 정하는 것은,
테마가 `button:hover` 에 짙은 색을 깔면 그 위의 짙은 글씨가 사라지기 때문이다.
...
메뉴에는 data URI 가 아니라 **파일 주소** 로 준다. data URI 로 주면 색이 사라진다.
`wp-admin/js/svg-painter.js` 가 base64 SVG 메뉴 아이콘을 받아 `fill` 을 전부 관리자 색상
하나로 바꿔 다시 칠하기 때문이다. 한 가지 색으로 그린 WordPress 기본 아이콘에는 맞는
처리이고, 이 아이콘에는 흰 구름만 남긴다. 주소를 주면 `<img>` 로 걸리고 그 script 는
손대지 않는다. 관리 화면마다 요청이 하나 늘지만 브라우저가 캐시한다.

관리 화면은 `<img>` 아이콘을 opacity 0.6 으로 눌러 두는데, 20px 짜리 글자에는 그만큼
대비가 깎인다. 이 메뉴 항목에만 1 로 되돌린다.

설정 화면 맨 위에도 56px 로 같은 그림이 걸린다. 파일이 없으면 기본 아이콘으로 내려가고 그
사실을 error log 에 남긴다.
